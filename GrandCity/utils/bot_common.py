...
def new_account(redis):
    ...
    pipe.set(f"uid:{uid}:slvr", 350000)
    pipe.set(f"uid:{uid}:gld", 2500)
    pipe.set(f"uid:{uid}:enrg", 100)
    pipe.set(f"uid:{uid}:exp", 9500)
...
def reset_account(redis, uid):
    pipe = redis.pipeline()
    pipe.set(f"uid:{uid}:slvr", 0)
    pipe.set(f"uid:{uid}:gld", 0)
    pipe.set(f"uid:{uid}:enrg", 100)
    pipe.set(f"uid:{uid}:exp", 1)
